...
func ResponseError(c *gin.Context, err error) {
	if errs, ok := err.(validator.ValidationErrors); ok {
		...
		for _, err := range fields {
			response.Resp().FailCode(c, code.ParamBindError, err)
			break
		}
	} else {
		errStr := err.Error()
		...
		if strings.ReplaceAll(strings.ToLower(errStr), " ", "") == "multipart:nextpart:eof" {
			response.Resp().FailCode(c, code.ParamBindError, "请根据要求填写必填项参数")
		} else {
			response.Resp().FailCode(c, code.ParamBindError, errStr)
		}
	}
}
